<template name="fever-popup">
	<view class="" style="z-index: 100;">
		<view :class="(type?'fever_popupShow':'fever_popupHide')+' fever_popup fever_flex'" id="fever_popup">
			<view v-if="type" :style="'opacity: '+(mark?0.3:0)+';'" class="'fever_popup_mark'" @click.stop.prevent="popupClose" @touchmove.stop.prevent="moveHandle"></view>
			<view v-if="type" class="fever_popup_def fever_flex" id="fever_popup_def" :style="'background-color: '+fun.rgb(0.8)+';'">
				...
				<view class="fever_popup_checkbox" v-if="checked">
					<checkbox-group @change="checkboxChange">
						<label>
							...
							<text>我已仔细阅读品牌的</text>
							<navigator class="fever_underline" url="#">隐私政策</navigator>
							<text>及</text>
...
<script>
	export default {
		...
		methods: {
			...
			onConfirm() {uni.showToast({title: "点击了确认",icon: "none",duration: 1500})},
